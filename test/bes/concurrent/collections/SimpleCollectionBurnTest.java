...
public class SimpleCollectionBurnTest
{
            ...
            for (int t = 0 ; t < threadCount ; t++)
            {
                new Thread(new Runnable()
                {
                    public void run()
                    {
                        ...
                        List<SimpleCollection.Item> items = new ArrayList<>();
                        while (System.nanoTime() < until)
                        {
                            ...
                            while (items.size() > targetSize)
                                items.remove(rand.nextInt(items.size())).delete();
    ...
    static boolean present(SimpleCollection<Object> c, Iterator<SimpleCollection.Item> check)
    {
        if (!check.hasNext())
            return true;
